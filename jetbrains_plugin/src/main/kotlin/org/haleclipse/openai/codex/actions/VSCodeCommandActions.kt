...
package org.haleclipse.openai.codex.actions
...
fun executeCommand(commandId: String, project: Project?, vararg args: Any?, hasArgs: Boolean? = true) {
    ...
    try {
        val pluginContext = project.getService(org.haleclipse.openai.codex.core.PluginContext::class.java)
        ...
        val proxy = rpcProtocol.getProxy(org.haleclipse.openai.codex.core.ServiceProxyRegistry.ExtHostContext.ExtHostCommands)
        if (proxy == null) {
 ...
 */
class OpenDevToolsAction(private val getWebViewInstance: () -> org.haleclipse.openai.codex.webview.WebViewInstance?) :
